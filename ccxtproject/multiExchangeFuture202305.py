...
USERCOINLIST=['FLOKI','KEY','GRT','PERP','COCOS','RDNT','WEMIX','BABYDOGE','ACS']
SPECIALCOINNAME={}
...
async def openorderPair(interval,book1,book2,exName1,exName2,coin,value,coinprice,type='profit'):
    ...
    if type == 'profit':
        ...
        if amount >0:
            ...
            input_coroutines=[]
            input_coroutines.append(quickOpenOrder(exchanges[exName1], value[exName1]['symbol'], book1['ask']*1.01, amount,1,'buy'))
            input_coroutines.append(quickOpenOrder(exchanges[exName2], value[exName2]['symbol'], book2['bid']*0.99, amount,1,'sell'))
            pairOrders = await asyncio.gather(*input_coroutines, return_exceptions=True)
            if pairOrders[0]:
                my_wallet[exName1]['USDT']['left']-= amount * coinprice
                my_wallet[exName1][coin]['left'] += amount
                pair['buyorder']['status'] = 'open'
                pair['buyorder']['id'] = pairOrders[0]['id']
            if pairOrders[1]:
                my_wallet[exName2][coin]['left'] -= amount
                my_wallet[exName2]['USDT']['left'] += amount*coinprice
    ...
    for key in futurePos.keys():
        ...
        for fupos in pairlist:
            ...
            if fupos['buyorder']['status']=='closed' and fupos['sellorder']['status']=='closed':
                    ...
                    realprofit = fupos['sellorder']['realprice']*fupos['sellorder']['realAmt']\
                                 -fupos['buyorder']['realprice']*fupos['buyorder']['realAmt']
                ...
                for ex1 in book:
                    for ex2 in book:
                        if not ex1 == ex2 and (ex1,ex2) in bookSreenWall[key]['diff']:
                            ...
                            if intervalB>-0.0001: #执行交易所迁移
                                str1=str('balance:'+str(key) + '@' + str(intervalB) + '@' + str(ex1) + '/' + str(ex2))
                                sendMsg(3, str1)
                                await openorderPair(interval,book[ex1],book[ex2],ex1,ex2,key,value['spot'],coinprice,'balance')
...
async def run():
    ...
    for key in bookSreenWall.keys():
        ...
        for ex in book['spot'].keys():
            book['spot'][ex]=None
            my_wallet[ex][key]= {'left':0,'offset':0}
