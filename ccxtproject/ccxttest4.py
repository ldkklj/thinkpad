    ...
    for key in records:
        ...
        if period:
            ...
            if averageDelta <-0.0005 and period[minidx]['sumdelta'] <-CONST_TRIGERRATIO:#
                #增加到预买单中
                #sendMsg(period[-10:-1])
                if not insertPriOrderlist('sell', key, period[minidx]['startprice'],period[minidx]['endprice'],tick):
                    continue
                #获取资产
    ...
    for key in records:
        ...
        if period:
            ...
            if averageDelta >0.0005  and period[maxidx]['sumdelta'] > CONST_TRIGERRATIO: #
                #增加预下单
                if not insertPriOrderlist('buy', key, period[maxidx]['startprice'],period[maxidx]['endprice'],tick):
                    continue
                #获取资产
                balance = getAccount(biance)
                if balance:
...
def createCoverOrder():
    for key in allTradeQueue:
        ...
        if newDistprice:
            if trade['side'] == 'buy':
                if newDistprice < trade['distPrice']   and newDistprice > trade['remainprice'] and abs(newDistprice-trade['distPrice'])>0.0002:
